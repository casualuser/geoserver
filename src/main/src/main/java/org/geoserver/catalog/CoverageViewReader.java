...
public class CoverageViewReader implements GridCoverage2DReader {
    ...
    static {
        ...
        try {
            ...
            if (LOGGER.isLoggable(Level.WARNING)) {
                LOGGER.warning("BandSelect operation unavailable. Band Select will be made through standard JAI BandSelect operations:\n "
                        + e.getLocalizedMessage());
            }
            BANDSELECT = null;
        }
    ...
    public GridCoverage2D read(GeneralParameterValue[] parameters) throws IllegalArgumentException,
        ...
        for (CoverageBand band : bands) {
            ...
            if (coverage != null) {
                ...
                int[] bandSelect = new int[]{Integer.valueOf(selectedBand)};
                if (BANDSELECT != null) {
                    final ParameterValueGroup param = BANDSELECT.getParameters();
                    param.parameter("Source").setValue(coverage);
                    param.parameter("SampleDimensions").setValue(bandSelect);
                    coverage = (GridCoverage2D) PROCESSOR.doOperation(param, hints);
                } else {
                    image = coverage.getRenderedImage();
                    image = BandSelectDescriptor.create(image, bandSelect, hints);
                    GridSampleDimension[] sampleDims = coverage.getSampleDimensions();
                    coverage = coverageFactory.create(coverageInfo.getName(), image,
                            coverage.getGridGeometry(),
                            new GridSampleDimension[]{sampleDims[bandSelect[0]]}, null, null);
                }
